...
Execute vector distance queries (15 min)
   1. Update application to handle vectorizing new records (use API endpoints for inserting new maintenance tasks.) - Have API endpoint for including user reviews 
   2. Execute various vector searches via the application
   3. Add new record and then search for it



To add vectors to your Azure Cosmos DB for NoSQL database's collections, you first need to generate embeddings using the `text-embedding-ada-002` model provided by Azure OpenAI. As part of the Bicep script you executed in Exercise 1, a deployment named `text-embedding-ada-002` was created using this model.
...
- [Generate vector embeddings with Azure OpenAI](https://learn.microsoft.com/azure/ai-services/openai/how-to/embeddings)
- [Azure Cosmos DB as a vector database](https://learn.microsoft.com/azure/cosmos-db/vector-database)
