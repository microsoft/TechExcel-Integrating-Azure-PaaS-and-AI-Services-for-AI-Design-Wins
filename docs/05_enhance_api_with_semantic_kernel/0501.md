...
TODO: Add `AddAzureOpenAITextEmbeddingGeneration(embeddingDeploymentName, endpoint, key)` to the `Kernel` singleton builder call added in Exercise 2 Task 2 (line 224 of the 0202.MD file), so the query text embedding functionality used in exercise 3 can be updated to use Semantic Kernel.

The builder with this new line will initialize and inject a built-in services from the Azure OpenAI Service. This Text Embedding service generates vectors used to perform vector searches against the Cosmos DB `MaintenanceRequests` container you implemented earlier.

- The steps to update the `VectorizationService` to use the built-it vector embeddings capabilities of Semantic Kernel:
  - Change the primary constructor for the `VectorizationService` to swap out the use of `AzureOpenAIClient` for Semantic Kernel.

    ```csharp
    public class VectorizationService(Kernel kernel, CosmosClient cosmosCient, IConfiguration configuration) : IVectorizationService
    ```

  - Replace the `_client` class variable with one that uses Semantic Kernel.
