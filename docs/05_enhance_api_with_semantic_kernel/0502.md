...
- To configure the maintenance request and database plugins:
  ...
  - Complete `Exercise 5 Task 2 TODO #6` and `#7` by adding Kernel function and Description descriptors of the `CreateMaintenanceRequest` and `SaveMaintenanceRequest` functions. The descriptors for each should look like:
...
- To enable the `/MaintenanceCopilotChat` endpoint on the API:
  ...
  - Complete `Exercise 5 Task 2 TODO #8` by calling the `Chat` method of the `MaintenanceCopilot`, passing in the user message from the request body. Return the response from the `Chat` method. The code in the API method should look like:
...
- To configure the **Chat Copilot** page as a conversational app in the Streamlit dashboard:
  - Complete `Exercise 5 Task 2 TODO #9` by setting `response` equal to the response from a POST request to the Copilot endpoint. The timeout should be set to 60 seconds or longer. The call should look like:
